...
def readVehicles(db):
    ...
    for r in db:
        if r[9] == 'active':
            ...
            vehicles[v]['currentCharge'] = int(r[2])/100.
            vehicles[v]['desiredCharge'] = int(r[3])/100.
...
def optimizer(vehicles):
    ...
    for v in vehicles:  #for each van i
        ...
        charging_now[v, 0] = 0 + ((vehicles[v]['lastChargingStatus']!=None or vehicles[v]['newStatus'] == 'Charging' ) and (battery_energy_current[v, 0] < battery_energy_capacity) )
